...
module.exports = async function (deployer, network, accounts) {
  ...
  try {
    ...
    await saveDeploymentInfo(network, {
      ...
      deploymentMethod: 'private-key-truffle'
    });
...
async function addSampleCourses(courseManager, instructor, config) {
  try {
    ...
    console.log("   添加 DeFi 协议开发课程...");
    await courseManager.addCourse(
      "DEFI_PROTOCOL",
      "DeFi 协议开发进阶",
      "深入学习去中心化金融协议开发，包括流动性挖矿、治理代币等",
      web3.utils.toWei("300", "ether"), // 300 YD 奖励
      3600 * 24 * 21, // 21天课程
      { 
        from: instructor,
        gas: config.transferGasLimit,
...
function printDeploymentSummary(info) {
  console.log("\n" + "=".repeat(60));
  console.log("🎯 部署摘要 (私钥安全部署)");
  console.log("=".repeat(60));
  console.log(`📡 网络: ${info.network}`);
  console.log(`🔑 部署方式: 私钥部署 (安全)`);
  ...
  console.log("\n🔒 安全提醒:");
  console.log("- 已使用私钥安全部署");
  console.log("- 请妥善保管部署账户私钥");
  console.log("- 建议启用合约暂停功能作为安全预案");
}
